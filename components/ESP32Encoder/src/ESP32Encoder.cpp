...
void ESP32Encoder::attach(int a, int b, encType et) {
	...
	if(! attachedInterrupt){
		...
		if (isrServiceCpuCore == ISR_CORE_USE_DEFAULT || isrServiceCpuCore == xPortGetCoreID()) {
			...
			if (er != ESP_OK){
				ESP_LOGE(TAG_ENCODER, "IPC call to install isr service on core %lu failed", isrServiceCpuCore);
			}
			if (ipc_ret_code != ESP_OK){
				ESP_LOGE(TAG_ENCODER, "Encoder install isr service on core %lu failed", isrServiceCpuCore);
			}
		}
#endif
