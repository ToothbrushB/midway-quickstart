...
bool Preferences::begin(const char *name, bool readOnly, const char *partition_label) {
  ...
  if (partition_label != NULL) {
    ...
    if (err) {
      ESP_LOGE(TAG, "nvs_flash_init_partition failed: %x",err);
      if (err == ESP_ERR_NVS_NO_FREE_PAGES) {
        ESP_LOGE(TAG, "No free pages in partition %s, erasing it", partition_label);
        err = nvs_flash_erase_partition(partition_label);
        if (err) {
          ESP_LOGE(TAG, "nvs_flash_erase_partition failed: %x",err);
          return false;
        }
        err = nvs_flash_init_partition(partition_label);
        if (err) {
          ESP_LOGE(TAG, "nvs_flash_init_partition failed after erase: %x",err);
          return false;
        }
      } else {
        return false;
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_open failed: %x",err);
    return false;
  }
  _started = true;
  return true;
...
bool Preferences::clear() {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_erase_all fail: %x",err);
    return false;
  }
  err = nvs_commit(_handle);
  if (err) {
    ESP_LOGE(TAG, "nvs_commit fail: %x",err);
    return false;
  }
  return true;
}
...
bool Preferences::remove(const char *key) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_erase_key fail: %s %x", key, err);
    return false;
  }
  err = nvs_commit(_handle);
  if (err) {
    ESP_LOGE(TAG, "nvs_commit fail: %s %x", key, err);
    return false;
  }
  return true;
}
...
size_t Preferences::putChar(const char *key, int8_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_i8 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putUChar(const char *key, uint8_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_u8 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putShort(const char *key, int16_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_i16 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putUShort(const char *key, uint16_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_u16 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putInt(const char *key, int32_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_i32 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putUInt(const char *key, uint32_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_u32 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putLong64(const char *key, int64_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_i64 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putULong64(const char *key, uint64_t value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_u64 fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putString(const char *key, const char *value) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_str fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
size_t Preferences::putBytes(const char *key, const void *value, size_t len) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_set_blob fail: %s %x", key, err);
    return 0;
  }
  err = nvs_commit(_handle);
  if (err) {
...
int8_t Preferences::getChar(const char *key, const int8_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_i8 fail: %s %x", key, err);
  }
  return value;
}
...
uint8_t Preferences::getUChar(const char *key, const uint8_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_u8 fail: %s %x", key, err);
  }
  return value;
}
...
int16_t Preferences::getShort(const char *key, const int16_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_i16 fail: %s %x", key, err);
  }
  return value;
}
...
uint16_t Preferences::getUShort(const char *key, const uint16_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_u16 fail: %s %x", key, err);
  }
  return value;
}
...
int32_t Preferences::getInt(const char *key, const int32_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_i32 fail: %s %x", key, err);
  }
  return value;
}
...
uint32_t Preferences::getUInt(const char *key, const uint32_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_u32 fail: %s %x", key, err);
  }
  return value;
}
...
int64_t Preferences::getLong64(const char *key, const int64_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_i64 fail: %s %x", key, err);
  }
  return value;
}
...
uint64_t Preferences::getULong64(const char *key, const uint64_t defaultValue) {
  ...
  if (err) {
    ESP_LOGV(TAG, "nvs_get_u64 fail: %s %x", key, err);
  }
  return value;
}
...
size_t Preferences::getString(const char *key, char *value, const size_t maxLen) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_get_str len fail: %s %x", key, err);
    return 0;
  }
  if (len > maxLen) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_get_str fail: %s %x", key, err);
    return 0;
  }
  return len;
}
...
size_t Preferences::getBytesLength(const char *key) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_get_blob len fail: %s %x", key, err);
    return 0;
  }
  return len;
}
...
size_t Preferences::getBytes(const char *key, void *buf, size_t maxLen) {
  ...
  if (err) {
    ESP_LOGE(TAG, "nvs_get_blob fail: %s %x", key, err);
    return 0;
  }
  return len;
}
...
size_t Preferences::freeEntries() {
  ...
  if (err) {
    ESP_LOGE(TAG, "Failed to get nvs statistics %x", err);
    return 0;
  }
