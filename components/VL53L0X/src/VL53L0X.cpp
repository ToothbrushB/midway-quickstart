...
static bool i2cDriverInstalled = false;
esp_err_t
VL53L0X::Done(i2c_cmd_handle_t i)
{
    i2c_master_stop(i);
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t val)
{
    ...
    i2c_master_write_byte(i, val, 1);
    esp_err_t err = Done(i);
    VL53L0X_LOG(TAG, "W %02X=%08lu %s", reg, val, esp_err_to_name(err));
}

uint8_t
VL53L0X::readReg8Bit(uint8_t reg)
{
    uint8_t buf[1] = {};
...
uint16_t
VL53L0X::readReg16Bit(uint8_t reg)
{
    uint8_t buf[2] = {};
    i2c_cmd_handle_t i = Read(reg);
...
const char *
VL53L0X::performSingleRefCalibration(uint8_t vhv_init_byte)
{
...
uint8_t
VL53L0X::getVcselPulsePeriod(vcselPeriodType type)
{
...
const char *
VL53L0X::getSpadInfo(uint8_t *count, int *type_is_aperture)
{
    uint8_t tmp;
...
const char *
VL53L0X::setMeasurementTimingBudget(uint32_t budget_us)
{
    SequenceStepEnables enables;
    SequenceStepTimeouts timeouts;
...
esp_err_t VL53L0X::config(i2c_port_t port, int8_t scl, int8_t sda, int8_t xshut, uint8_t address, uint8_t io_2v8)
{
    printf("A\n");
    ...
    if (xshut >= 0)
    {
        gpio_reset_pin(static_cast<gpio_num_t>(xshut));
        gpio_set_level(static_cast<gpio_num_t>(xshut), 0); // Off
        gpio_set_drive_capability(static_cast<gpio_num_t>(xshut), GPIO_DRIVE_CAP_3);
        gpio_set_direction(static_cast<gpio_num_t>(xshut), GPIO_MODE_OUTPUT);
    }
    this->xshut = xshut;
    this->io_2v8 = io_2v8;
    this->port = port;
...
VL53L0X::init(uint8_t address)
{
    ...
    {                                // XSHUT or power control
        gpio_set_level(static_cast<gpio_num_t>(xshut), 0); // Off
        usleep(100000);
        gpio_set_level(static_cast<gpio_num_t>(xshut), 1); // On
...
void VL53L0X::setTimeout(uint16_t new_timeout)
{
    io_timeout = new_timeout;
}
...
uint16_t
VL53L0X::readRangeContinuousMillimeters()
{
    startTimeout();
    while ((readReg8Bit(RESULT_INTERRUPT_STATUS) & 0x07) == 0)
    {
